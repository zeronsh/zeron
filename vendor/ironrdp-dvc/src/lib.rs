...
pub trait DvcProcessor: AsAny + Send {
    ...
    /// Maximum incoming reassembled data size, excluding DVC/SVC framing.
    /// Set when the channel is created; `None` retains the upstream behavior.
...
impl DynamicVirtualChannel {
    fn from_boxed(processor: Box<dyn DvcProcessor + Send>) -> Self {
        let complete_data = CompleteData::new(processor.max_message_size());
        Self {
