...
impl StaticVirtualChannel {
    ...
    /// Bound incoming message reassembly (excluding the SVC header).
    /// Configure before receiving data. Changing the limit discards partial data.
    pub fn set_max_message_size(&mut self, maximum: usize) {
        self.chunk_processor.chunked_pdu = Vec::new();
        self.chunk_processor.max_message_size = Some(maximum);
    }
...
impl ChunkProcessor {
    ...
    fn dechunkify(&mut self, payload: &[u8]) -> DecodeResult<Option<Vec<u8>>> {
        ...
        if let Some(maximum) = self.max_message_size {
            let declared = usize::try_from(header.length).ok();
            let accumulated = self.chunked_pdu.len().checked_add(cursor.remaining().len());
            if declared.is_none_or(|size| size > maximum) || accumulated.is_none_or(|size| size > maximum) {
                // Release the allocation before returning the fatal protocol error.
                self.chunked_pdu = Vec::new();
                return Err(ironrdp_core::invalid_field_err!("SVC", "length", "message exceeds configured limit"));
            }
        }
