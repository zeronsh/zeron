...
pub fn tool_group_summary(tools: &[ToolItem], locale: Locale) -> String {
    ...
    match (base.is_empty(), thoughts) {
        (_, 0) => base,
        (true, 1) => i18n::translate(MessageId::TranscriptThoughtProcess, locale).to_string(),
        (true, n) => i18n::fill(
        ...
        (false, 1) => i18n::fill(
            MessageId::TranscriptThoughtWithBase,
            "{base}",
            &base,
            locale,
        ),
        (false, n) => i18n::fill_many(
            MessageId::TranscriptThoughtTimesWithBase,
            &[("{n}", &n.to_string()), ("{base}", &base)],
            locale,
        ),
    }
}
...
fn chip_header_row(
    ...
    let (label, detail) = if tool.is_thought {
        ...
        tool_chip_text(&tool.call, i18n::locale(cx))
    };
