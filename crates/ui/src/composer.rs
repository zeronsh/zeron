...
pub(crate) use presentation::RADIUS as COMPOSER_RADIUS;
...
use presentation::MAX_WIDTH as COMPOSER_MAX_WIDTH;
...
// Lexical inclusion preserves the native namespace, privacy and existing tests.
include!("composer/input.rs");
...
impl Composer {
    ...
    /// Browser file picks are bytes, never synthetic host paths. Stage first,
    /// then mutate the existing per-chat strip only after every selected file
