...
fn front_page_mirrors_the_desktop_sidebar() {
    ...
    assert_eq!(ws.session("chat-tabs").unwrap().section_id.as_deref(), Some("section-p0"));
...
fn sidebar_writes_reorder_the_front_page() {
    ...
    client.assign_section("chat-deploy", Some(section.clone())).unwrap();
    let ws = client.workspace();
    ...
    assert_eq!(client.workspace().session("chat-cjk").unwrap().title, "Renamed");
...
fn send_streams_a_reply_and_adopts_the_echo() {
    ...
    let outcome = session.send(SendRequest::text("Explain the pipeline")).unwrap();
    ...
    assert!(rx.recv_timeout(Duration::from_secs(1)).is_ok(), "watch delivers the current snapshot");
...
fn sends_while_working_park_on_the_queue_and_drain() {
    ...
    wait_for("idle", Duration::from_secs(30), || !session.snapshot().working);
}
...
fn questions_answer_through_respond_input() {
    let (client, _dir) = demo(fast());
    let session = client.open_session("chat-deploy").unwrap();
    session.send(SendRequest::text("please ?ask first")).unwrap();
...
fn interrupt_aborts_the_live_turn() {
    ...
    wait_for("streaming", Duration::from_secs(5), || session.snapshot().streaming);
...
fn new_sessions_are_born_on_chat2() {
    ...
    session.send(SendRequest::text("Deploy the landing page")).unwrap();
    wait_for("titled from the first prompt", Duration::from_secs(5), || {
...
fn warm_sessions_are_capped_and_preload_follows_the_front_page() {
    ...
    for id in ["chat-cjk", "chat-deploy", "chat-blog", "chat-ios-scroll", "chat-ios-keyboard"] {
        client.open_session(id).unwrap();
        client.close_session(id);
    }
    let open = client.open_session_ids();
    assert!(open.len() <= zeron_client::WARM_SESSION_CAP + 2, "{open:?}");
    assert!(open.iter().any(|o| o == "chat-home"), "attached sessions stay");
    assert!(open.iter().any(|o| o == "chat-veil"), "streaming sessions stay");
}
...
fn offline_sends_queue_durably_and_deliver_on_recovery() {
    ...
    let SendOutcome::Started { message_id } = session.send(SendRequest::text("while offline")).unwrap()
    else {
    ...
    assert_eq!(
        client.workspace().session("chat-deploy").unwrap().send_state,
        Some(zeron_client::SendState::Queued)
    );
    assert!(session.snapshot().entry(&message_id).unwrap().echo.is_some());
    client.set_network_online(true);
    wait_for("delivered after recovery", Duration::from_secs(5), || {
        session.snapshot().entry(&message_id).is_some_and(|e| e.echo.is_none())
    });
    wait_for("send state clears", Duration::from_secs(5), || session.composer().send_state.is_none());
}
