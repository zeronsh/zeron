...
#   target/package/glitch-flow-<version>-macos-<arch>.dmg          (user download)
#   target/package/glitch-flow-<version>-macos-<arch>-app.tar.gz   (auto-updater)
# containing Glitch Flow.app (unsigned unless CODESIGN_IDENTITY is set).
#
...
APP="$OUT_DIR/Glitch Flow.app"
DMG="$OUT_DIR/glitch-flow-$VERSION-macos-$ARCH.dmg"
APP_TARBALL="$OUT_DIR/glitch-flow-$VERSION-macos-$ARCH-app.tar.gz"
...
install -m 755 "$ROOT/target/release/glitch-flow" "$APP/Contents/MacOS/glitch-flow"
...
# Icon: iconset from the Glitch Flow artwork with a pre-applied macOS mask.
ICONSET="$OUT_DIR/glitch-flow.iconset"
...
iconutil -c icns "$ICONSET" -o "$APP/Contents/Resources/glitch-flow.icns"
rm -rf "$ICONSET"
...
if $NOTARIZE; then
  ...
  # Gatekeeper offline.
  ZIP="$OUT_DIR/glitch-flow-notarize.zip"
...
tar -czf "$APP_TARBALL" -C "$OUT_DIR" "Glitch Flow.app"
...
dmgbuild.build_dmg(
    ...
    volume_name="Glitch Flow",
    settings={
        "format": "UDZO",
        "files": [app],
        "symlinks": {"Applications": "/Applications"},
        "icon": os.path.join(app, "Contents/Resources/glitch-flow.icns"),
        ...
        "icon_size": 104,
        "text_size": 12,
        "icon_locations": {"Glitch Flow.app": (165, 195), "Applications": (495, 195)},
    },
)
PY
rm -f "$BG_TIFF"
