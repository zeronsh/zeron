...
Push-Location $root
try {
    cargo build --release --locked -p glitch-flow
    ...
    $probe.FileName = (Resolve-Path -LiteralPath './target/release/glitch-flow.exe').Path
    ...
    try {
        ...
        $versionMatch = [regex]::Match($stdout.Result.Trim(), '\Aglitch-flow (\d+\.\d+\.\d+)\z')
    ...
    $stage = Join-Path $out "glitch-flow-$version-windows-x86_64"
    New-Item -ItemType Directory -Force -Path $stage | Out-Null
    Copy-Item -LiteralPath './target/release/glitch-flow.exe' -Destination (Join-Path $stage 'glitch-flow.exe')
    if ($ReleasesUrl) {
        @{ releases_url = $ReleasesUrl } | ConvertTo-Json | Set-Content -Encoding utf8NoBOM -LiteralPath (Join-Path $stage 'glitch-flow-update.json')
    }
    ...
    Copy-Item -LiteralPath './target/release/glitch-flow.exe' -Destination "$stage.exe"
